...
New path is defined and dubins curve local planner is used. Dubins curve planner will try to find a path from the initial point to the goal. RRT line 7 will compute the collision check which will include either robot collision with obstacles (which are inflated for this purpose) or with the arena border. If there is collsion, robot will try to make a curve around the obstacle or the arena border and find the new path. 
...
![RRT SECONDARY FLOW](imgs/blocks/RRT.png)
